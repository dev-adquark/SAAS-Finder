...
export function CategoryArt({ slug }: { slug: string }) {
  switch (slug) {
    case "website-builders":
      return (
        <svg viewBox="0 0 280 150" aria-hidden="true">
          ...
          <circle className="a3" cx="32" cy="26" r="3" /><circle className="a2" cx="44" cy="26" r="3" />
          <g className="anim"><rect className="a3" x="34" y="48" width="96" height="44" rx="3" /><rect className="a5" x="140" y="48" width="106" height="20" rx="3" /><rect className="a5" x="140" y="72" width="70" height="20" rx="3" /></g>
    ...
    case "design":
      return (
        <svg viewBox="0 0 280 150" aria-hidden="true">
          ...
          <g className="anim"><circle className="a3" cx="112" cy="76" r="42" /></g>
          <g className="anim-y"><rect className="a2" x="134" y="40" width="72" height="72" rx="4" transform="rotate(12 170 76)" /></g>
          <polygon className="a1" points="196,30 250,118 142,118" />
          <path className="a4" d="M30 132 L250 18" strokeDasharray="3 5" />
          <circle className="a1" cx="112" cy="76" r="54" strokeDasharray="1 5" />
        </svg>
      );
    case "crm": {
      ...
      return (
        <svg viewBox="0 0 280 150" aria-hidden="true">
          {edges.map(([a, b], i) => <line key={i} className="a1" x1={nodes[a][0]} y1={nodes[a][1]} x2={nodes[b][0]} y2={nodes[b][1]} />)}
          <g className="anim">{nodes.map(([x, y], i) => <circle key={i} className={i === 7 ? "a3" : i % 3 === 0 ? "a2" : "a5"} cx={x} cy={y} r={i === 7 ? 14 : 8} />)}</g>
    ...
    case "marketing":
      return (
        <svg viewBox="0 0 280 150" aria-hidden="true">
          {[0, 1, 2, 3].map((i) => <path key={i} className={i % 2 ? "a4" : "a1"} d={`M10 ${75 + (i - 1.5) * 16} C 60 ${30 + i * 8}, 110 ${120 - i * 8}, 160 ${75 + (i - 1.5) * 10} S 250 ${40 + i * 12}, 272 ${70 + i * 6}`} />)}
          <g className="anim-y">{[40, 70, 100, 130, 160, 190, 220, 250].map((x, i) => <rect key={x} className={i === 5 ? "a3" : "a5"} x={x - 6} y={130 - (12 + ((i * 37) % 60))} width="12" height={12 + ((i * 37) % 60)} rx="2" />)}</g>
          <circle className="a2" cx="190" cy="54" r="6" />
        </svg>
      );
